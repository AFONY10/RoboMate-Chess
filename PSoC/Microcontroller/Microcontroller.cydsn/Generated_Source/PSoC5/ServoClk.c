...
void ServoClk_Start(void) 
{
    ...
    ServoClk_CLKEN |= ServoClk_CLKEN_MASK;
	ServoClk_CLKSTBY |= ServoClk_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: ServoClk_Stop
********************************************************************************
*
* Summary:
...
void ServoClk_Stop(void) 
{
    ...
    ServoClk_CLKEN &= (uint8)(~ServoClk_CLKEN_MASK);
	ServoClk_CLKSTBY &= (uint8)(~ServoClk_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: ServoClk_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((ServoClk_CLKEN & ServoClk_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = ServoClk_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = ServoClk_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* ServoClk__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(ServoClk_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(ServoClk_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: ServoClk_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: ServoClk_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = ServoClk_GetSourceRegister();
    uint16 oldDivider = ServoClk_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = ServoClk_CLKEN & ServoClk_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((ServoClk_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: ServoClk_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(ServoClk_DIV_PTR);
}


/*******************************************************************************
* Function Name: ServoClk_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    ServoClk_MOD_SRC |= modeBitMask & (uint8)ServoClk_MODE_MASK;
}


/*******************************************************************************
* Function Name: ServoClk_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    ServoClk_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(ServoClk_MODE_MASK));
}


/*******************************************************************************
* Function Name: ServoClk_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return ServoClk_MOD_SRC & (uint8)(ServoClk_MODE_MASK);
}


/*******************************************************************************
* Function Name: ServoClk_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = ServoClk_GetDividerRegister();
    uint8 oldSrc = ServoClk_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        ServoClk_MOD_SRC |= CYCLK_SSS;
        ServoClk_MOD_SRC =
            (ServoClk_MOD_SRC & (uint8)(~ServoClk_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: ServoClk_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return ServoClk_MOD_SRC & ServoClk_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: ServoClk_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    ServoClk_PHASE = clkPhase & ServoClk_PHASE_MASK;
}


/*******************************************************************************
* Function Name: ServoClk_GetPhase
********************************************************************************
*
* Summary:
...
{
    return ServoClk_PHASE & ServoClk_PHASE_MASK;
}
